...
function Equalizer({ playPauseSwitch }) {
   ...
   // Создаем инструменты анализа звука
   useEffect(() => {
      // Обходим баг - обеспечиваем кроссбраузерность
      audioContext.current = new (window.AudioContext || window.webkitAudioContext)()

      // Создаем аудиоузел
      if (audio.children) {
         audioNodeSource.current = audioContext.current.createMediaElementSource(audio);
